...
CLIENT_ID = "YA24970"       # Your Fyers Client ID
PIN = "0312"                # User pin for Fyers account
APP_ID = "UKNEWOZLX7"        # App ID from MyAPI dashboard (https://myapi.fyers.in/dashboard). The format is appId-appType. 
# Example: YIXXXXXSE-100. In this code, YIXXXXXSE is the APP_ID and 100 is the APP_TYPE
APP_TYPE = "100"
APP_SECRET = "XZS2DTC4V0"   # App Secret from myapi dashboard (https://myapi.fyers.in/dashboard)
TOTP_SECRET_KEY = "GPW75J6I524P36R5NTJR734CIFWLU3AG"  # TOTP secret key, copy the secret while enabling TOTP.
...
def main():
    ...
    directory = r"C:\Users\arushi\Desktop\My work\fish_strategy\fyers_live_code\multifish_code"

    with open(f"{directory}\\fyers_appid.txt", 'w') as file:
        file.write(appid1)
        print(f'Appid has been saved in {directory} -> fyers_appid.txt')

    with open(f"{directory}\\fyers_token.txt", 'w') as file:
        file.write(token1)
        print(f'Token has been saved in {directory} -> fyers_token.txt')
